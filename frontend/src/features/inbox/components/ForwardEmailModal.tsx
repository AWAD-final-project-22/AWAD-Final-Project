...
interface ForwardEmailModalProps {
  ...
  forwardParams?: IForwardEmailParams;
...
export const ForwardEmailModal: React.FC<ForwardEmailModalProps> = ({
  ...
  useEffect(() => {
    if (open && forwardParams) {
      form.setFieldsValue({
        to: forwardParams.to || [],
        cc: forwardParams.cc || [],
        bcc: forwardParams.bcc || [],
        subject: originalSubject?.startsWith('Fwd:')
          ? originalSubject
          : `Fwd: ${originalSubject || ''}`,
        body: forwardParams.includeOriginal
          ? `\n\n---------- Forwarded message ----------\n${forwardParams.body}`
          : '',
      });
    }
  }, [open, forwardParams, originalSubject, form]);
  ...
  return (
    ...
    >
      <ModalHeader>
        <HeaderTitle>Forward Email</HeaderTitle>
        <HeaderActions>
